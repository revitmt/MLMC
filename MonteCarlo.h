...
using namespace deterministic_solver;
...
/*---------------------------------------------------------------------------*/
	...
	{
		public:
			typedef Problem<dim> problem_type;
...
/*---------------------------------------------------------------------------*/
	...
	MonteCarloSolver<dim>::MonteCarloSolver( int l, double lin_sol_err, int n, correction_or_base mode_ )
	{
		...
		if ( num_of_samples != 0 )
		{
			...
			{
				case test:
					coarse_Problem.reinit(regular, level, lin_sol_err);
					break;
				case base:
					coarse_Problem.reinit(regular, level, lin_sol_err);
					break;
				case correction:
//					if ( level == 1 )
//						coarse_Problem.reinit(regular, std::max(0,level-1), lin_sol_err);
//					else
						coarse_Problem.reinit(coarse, std::max(0,level-1), lin_sol_err);
					fine_Problem.reinit(fine, level, lin_sol_err);
	...
	void MonteCarloSolver<dim>::run( solution_type<dim> &MC_solution, solution_type<dim> &variance, int num_of_samples_old )
	{
		if ( num_of_samples != 0 )
		{
			if ( mode == test )
				...
				{
					coefficient_function.generate();

					level_solution.sample = -(i+num_of_samples_old);
					coarse_Problem.run( &coefficient_function, level_solution);

					coarse_info = coarse_Problem.get_solver_info();

//					if ( num_of_samples > 1 )
						allSolutions<dim>::add( coefficient_function.get_coefficients(), level_solution, -(i+num_of_samples_old) );
